...
public class ROSClockPublisher : MonoBehaviour
{
    ...
    bool ShouldPublishMessage =>
        Clock.FrameStartTimeInSeconds - PublishPeriodSeconds > m_LastPublishTimeSeconds;
    ...
    void OnValidate()
    {
        ...
        {
            Debug.LogWarning(
                "Found too many clock publishers in the scene, there should only be one!");
        }
    ...
    void PublishMessage()
    {
        ...
        var clockMsg = new TimeMsg { sec = (int)publishTime,
                                     nanosec = (uint)((publishTime - Math.Floor(publishTime)) *
                                                      Clock.k_NanoSecondsInSeconds) };
