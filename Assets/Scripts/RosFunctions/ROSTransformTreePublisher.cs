...
public class ROSTransformTreePublisher : MonoBehaviour
{
    ...
    {
        // TODO: Some of this could be done once and cached rather than doing from
        // scratch every time Only generate transform messages from the children,
    ...
    void PublishMessage()
    {
        ...
        if (m_GlobalFrameIds.Count > 0)
        {
            var tfRootToGlobal = new TransformStampedMsg(
                ...
                m_TransformRoot.name, m_TransformRoot.Transform.To<FLU>());
        ...
        // In case there are multiple "global" transforms that are effectively the
        // same coordinate frame, treat this as an ordered list, first entry is the
        // "true" global
