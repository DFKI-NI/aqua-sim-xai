...
public class ROSCameraDataPublisher : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            renderTexture =
                new RenderTexture(Camera.main.pixelWidth, Camera.main.pixelHeight, 0,
    ...
    {
        RenderTexture lastActive = RenderTexture.active; // Store the current active RenderTexture
        Camera.main.targetTexture = renderTexture;

        RenderTexture.active = renderTexture; // Set renderTexture as active
        Camera.main.Render();
        ...
        RenderTexture.active = lastActive; // Restore the original active RenderTexture

        byte[] rawData = mainCameraTexture.GetRawTextureData();
        Destroy(mainCameraTexture); // Release mainCameraTexture

        return rawData;
    }

    private void OnDestroy() { Destroy(mainCameraTexture); }
}