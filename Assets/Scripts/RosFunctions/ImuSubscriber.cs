...
public class ImuSubscriber : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Math.Abs(sinp) >= 1)
        {
            ...
            pitch = Math.Abs(Math.PI) * Math.Sign(sinp); // Math.CopySign(Math.PI / 2, sinp); // use
                                                         // 90 degrees if out of range
        }
        else
            pitch = Math.Sin(sinp);
        // yaw (z-axis rotation)
        ...
        currentChangeVelocity +=
            currentAcceleration * Time.deltaTime; // * Time.deltaTime* Time.deltaTime;
