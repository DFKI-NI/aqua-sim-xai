...
public class SimpleCameraController : MonoBehaviour
{
    ...
    {
        // try to compensate the diff between the two input systems by multiplying
        // with empirical values
#if ENABLE_INPUT_SYSTEM
        var delta = lookAction.ReadValue<Vector2>();
        delta *= 0.5f; // Account for scaling applied directly in Windows code by
                       // old input system.
    ...
    bool IsCameraRotationAllowed()
    {
        ...
        canRotate |=
            Gamepad.current != null ? Gamepad.current.rightStick.ReadValue().magnitude > 0 : false;
        return canRotate;
#else
